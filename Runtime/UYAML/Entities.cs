...
namespace Lachee.UYAML
{
    ...
    public class UProperty
    {
        public string name;
        public UNode value;
        public UProperty(string name, UNode value)
        {
            this.name = name;
            this.value = value;
        }
    ...
    public class UObject : UNode, IUPropertyCollection, IDictionary<string, UNode>
    {
        public Dictionary<string, UProperty> properties = new Dictionary<string, UProperty>();

        public UNode this[string name]
        {
            ...
            set
            {
                if (properties.TryGetValue(name, out var p))
                    ...
                    properties.Add(name, new UProperty(name, value));
            }
        }

        public ICollection<string> Keys => properties.Keys;
        public ICollection<UNode> Values => properties.Values.Select(v => v.value).ToList();
        ...
        public void Add(string key, UNode value)
            => properties.Add(key, new UProperty(key, value));

        public void Add(KeyValuePair<string, UNode> item)
            => Add(item.Key, item.Value);

        public void Clear()
            => properties.Clear();

        public bool Contains(KeyValuePair<string, UNode> item)
            => properties.TryGetValue(item.Key, out var p) && p.value == item.Value;

        public bool ContainsKey(string key)
            => properties.ContainsKey(key);

        public void CopyTo(KeyValuePair<string, UNode>[] array, int arrayIndex)
            => throw new System.NotImplementedException();
        

        public IEnumerator<KeyValuePair<string, UNode>> GetEnumerator()
            => properties.Select(kp => new KeyValuePair<string, UNode>(kp.Key, kp.Value.value)).GetEnumerator();

        public bool Remove(string key)
            => properties.Remove(key);

        public bool Remove(KeyValuePair<string, UNode> item)
        {
            if (properties.TryGetValue(item.Key, out var i2) && i2.value == item.Value)
                return properties.Remove(item.Key);
            return false;
        }

        public bool TryGetValue(string key, out UNode value)
        {
            value = default;
    ...
    public sealed class UComponent : UNode, IUPropertyCollection
    {
        ...
        public ClassIdentifier classID = ClassIdentifier.Object;

        public string name => _property?.name;
        public UObject root => (UObject)_property?.value;
        public UProperty rootProperty => _property;
